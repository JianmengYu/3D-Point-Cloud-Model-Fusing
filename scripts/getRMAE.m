...
for i=1:50
    ...
    if max(points) == 1
        error(i,3) = sqrt(mean(abs(localError(points==1))));
    end
    
    if max(points) == 2
        localError1 = sqrt(mean(abs(localError(points==1))));
        localError2 = sqrt(mean(abs(localError(points==2))));
    ...
    if max(points) == 3
        localError1 = sqrt(mean(abs(localError(points==1))));
        localError2 = sqrt(mean(abs(localError(points==2))));
        localError3 = sqrt(mean(abs(localError(points==3))));
...
set(gca,'xTick',1:50);
xlabel('Frame Number');
ylabel('(Average) Root Mean Absolute Error');
hold off;